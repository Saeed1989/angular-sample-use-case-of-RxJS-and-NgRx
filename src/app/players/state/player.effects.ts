...
export class PlayerEffects {
    ...
    return this.actions$.pipe(
      ...
      mergeMap(() =>
        this.playerService.getPlayers().pipe(
          ...
          catchError((error) =>
            of(PlayerApiActions.loadPlayersFailure({ error }))
          )
        )
      )
    );
  ...
  updatePlayer$ = createEffect(() => {
    return this.actions$.pipe(
      ...
      concatMap((action) =>
        this.playerService.updatePlayer(action.player).pipe(
          map((player) => PlayerApiActions.updatePlayerSuccess({ player })),
          catchError((error) =>
            of(PlayerApiActions.updatePlayerFailure({ error }))
          )
        )
      )
    );
    ...
    return this.actions$.pipe(
      ...
      concatMap((action) =>
        this.playerService.createPlayer(action.player).pipe(
          ...
          catchError((error) =>
            of(PlayerApiActions.createPlayerFailure({ error }))
          )
        )
      )
    );
    ...
    return this.actions$.pipe(
      ...
      mergeMap((action) =>
        this.playerService.deletePlayer(action.playerId).pipe(
          map(() =>
            PlayerApiActions.deletePlayerSuccess({ playerId: action.playerId })
          ),
          catchError((error) =>
            of(PlayerApiActions.deletePlayerFailure({ error }))
          )
        )
      )
    );
